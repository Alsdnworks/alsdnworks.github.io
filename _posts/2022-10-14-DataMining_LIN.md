...
중요도는 t검정을 통해 알 수 있다. t검정은 다음과 같이 정의된다.
...
이항형 목표변수 값을 $Y_i$라고 하면 $Y_i$의 값은 0 또는 1이다. 목표변수가 1을 가질 확률을 ${\pi = Pr(Y_i = 1)}$ 로 정의할때 로지스틱회귀모형은 다음과 같다.

$$Y=\beta_0 + \beta_1X_{1i} + \beta_2X_{2i} + ... + \beta_pX_{pi} , i=1,...n$$
$$logit({\pi})_i = \frac{exp(Y)}{1+exp(Y)}, i=1,...n$$

X가 입력변수의 값들이고 이항형목표변수는 이항분포를 따른다고 가정할때 다음과 같이 표기 가능

$$log(\frac{\pi_i}{1-\pi_i})=Y$$
...
- 성공실패의 비를 odds Ratio라고 하고 이에 로그를 취한것을 로짓변환이라고 부른다.
...
여러 간으한 임계치에 대해 민감도와 특이도를 계산하여 ROC 곡선을 그린다. ROC 곡선은 민감도와 특이도의 관계를 나타내는 곡선으로, 민감도와 특이도가 높을수록 예측력이 좋다고 말 할 수있기때문에 좌상단에 가까울수록 곡선 아래 면적이 커진다. 이 면적을 AUC(Area Under the Curve)라고 한다. AUC는 0.5에서 1.0 사이의 값을 가지며, 1에 가까울수록 예측력이 높다고 할 수 있다.
